    ...
    def _on_rollout_start(self):
        ...
        for image in images:
            ...
            if self.display:
                ...
                images_to_display = first_half.permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
                for img in images_to_display:
                    cv2.imshow('Processed RGB Image', img)
            ...
            if self.display:
                # Convert to NumPy and display
                images_to_display = rgb_transform.permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
                for img in images_to_display:
                    cv2.imshow('Processed RGB Image', img)
                    if cv2.waitKey(0) & 0xFF == ord('q'):  # Press 'q' to close the window
                        break
            ...
            for i in range(second_half.size(0)):
                ...
                cv2.imshow('processed Image', enhanced_transform[i].numpy())
