...
class KorniaAugmentationCallback(BaseCallback):
    ...
    def _on_rollout_end(self):
        ...
        for image in images:
            ...
            idx = np.random.choice(len(self.augment_images), size=second_half.size(0), replace=True)
            alpha = np.random.uniform(0.7, 1, size=second_half.size(0))
            beta, gamma = 1 - alpha, np.zeros(second_half.size(0))

            self.augment_images = np.array(self.augment_images)

            print(self.augment_images[idx])

            enhanced_transform = KEnhance.add_weighted(second_half, alpha[:, None, None, None], torch.from_numpy(self.augment_images[idx]), beta[:, None, None, None], gamma[:, None, None, None])
