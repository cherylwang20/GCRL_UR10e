from typing import Any, Optional, Union, List, Type, Dict, Tuple
...
class CustomDictFeaturesExtractor(BaseFeaturesExtractor):
    ...
    def forward_conv(self, obs, flatten = True):
        ...
        conv = th.cat([conv_current, conv_prev], axis=1)
    ...
    def calculate_feature_dim(self, cnn, channels, height, width):
        with th.no_grad():
            sample_input = th.randn(1, channels, height, width)
    ...
    def forward(self, observations):
        ...
        combined_rgb = th.cat([rgb1, rgb2, rgb3], dim=3).permute(0, 3, 1, 2).float()
        ...
        stacked_features_tensor = th.cat([rgb_features, binary_features], dim=1)
        ...
        concatenated_features = th.cat((stacked_features_tensor, mlp_output), dim=1)
...
class SACPolicy(BasePolicy):
    ...
    def __init__(
        ...
        use_expln: bool = False,
        clip_mean: float = 2.0,
        features_extractor_class: Type[BaseFeaturesExtractor] = CustomDictFeaturesExtractor,
...
class MultiInputPolicySAC(SACPolicy):
    """
    Policy class (with both actor and critic) for SAC.
