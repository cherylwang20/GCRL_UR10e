...
class BasicReachGrasp(gym.Env):
    ...
    def __init__(self, channel = 4, render_mode=None):
        ...
        self.nwidth = 120
        self.nheight = 212
        self.action_space = gym.spaces.Box(act_low, act_high, dtype=np.float64) # clockwise or counterclockwise, for each of the 5 moving joints
        self.observation_space = gym.spaces.Dict({
            'image': gym.spaces.Box(low=0, high=255, shape=(self.nwidth, self.nheight, self.channel), dtype=np.float64),  # Use np.float32 here
            'vector': gym.spaces.Box(obs_range[0]*np.ones(14), obs_range[1]*np.ones(14), dtype=np.float64)  # Ensure consistency in dtype usage
        })
    
    def get_obs_dict(self):
        self.obs_dict['qp_robot'] = self.receive.getActualQ() + [0.159 ]
        self.obs_dict['qv_robot'] = np.concatenate((self.vel_action[:6].copy(), [0]))
        self.obs_dict['reach_err'] = np.array(self.duck_pos) - np.array(self.receive.getActualTCPPose()[:3]) #use the end_effector to check
    ...
    def get_reward_dict(self, obs_dict):
        ...
        rwd_dict = collections.OrderedDict((
            ...
            ('done', np.array([contact == True])), #    obj_height  - self.obj_init_z > 0.2, #reach_dist > far_th
        ))
    ...
    def vtp_step(self, ctrl_desired, last_qpos, n_frames = 1):
        ...
        control = last_qpos + control_v*dt
        ctrl_feasible = np.clip(control, self.robot_pos_bound[:7, 0], self.robot_pos_bound[:7, 1])

        for _ in range(n_frames):
            protective_stop = self.moveJ(ctrl_feasible[:6])
            self.gripper.move_and_wait_for_pos(0, 25, 25)


        return ctrl_feasible, control_v
    
    def moveJ(self, new_vel):
        success = self.control.moveJ(new_vel, speed=1.25, acceleration = 0.8)
        #time.sleep(0.01)
        while not success:
            if self.receive.isProtectiveStopped(): 
                ...
                self.reconnect()
                success = self.control.moveJ(new_vel, speed=0.25, acceleration = 0.01)
                time.sleep(0.4)
    ...
    def step(self, a):
        ...
        #self.final_image = self.image_state
        if self.done:
            self.reconnect()
    ...
    def forward(self):
        ...
        #print(image.size)
        obs = {'image': self.current_image.reshape((self.nwidth, self.nheight, self.channel)), 'vector': obsvec}
    ...
    def get_mask(self, rgb):
        ...
        mask = cv.resize(mask, dsize=(self.nheight, self.nwidth), interpolation=cv.INTER_CUBIC)

        self.image_state = cv.resize(rgb, dsize=(self.nheight, self.nwidth), interpolation=cv.INTER_CUBIC)
    ...
    def reset(self, seed=None):
        ...
        if not success:
            raise ValueError("Move failed in reset function")
        ...
        target_names = ['plastic graduated cylinder', 'plastic beaker', 'test tube']#['toy robot', 'green frog', 'water bottle']#'red cube','green apple', 'yellow rubber duck'] #'toy rubber duck', 'plastic cylinder', 
        ...
        self.final_image = np.ones((self.nwidth, self.nheight, self.channel), dtype=np.uint8)
        self.step_count = 0
        self.done = False
