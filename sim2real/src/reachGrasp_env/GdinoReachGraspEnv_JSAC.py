...
class BasicReachGrasp(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.observation_space = gym.spaces.Dict({
            'image': gym.spaces.Box(low=0, high=255, shape=(self.nwidth, self.nheight, 4), dtype=np.float32),  # Use np.float32 here
    ...
    def speedJ(self, new_vel):
        success = self.control.speedJ(new_vel, acceleration = 4.5, time = 0)
        time.sleep(self.DT)
        while not success:
    ...
    def forward(self):
        ...
        self.current_image = np.concatenate((self.image_state, np.expand_dims(self.mask_image, axis=-1)), axis=2)


        # returns obs(t+1), rwd(t+1), done(t+1), info(t+1)
        #print(image.size)
        obs = {'image': self.current_image.reshape((self.nwidth, self.nheight, 4)), 'vector': obsvec}
    ...
    def get_mask(self, rgb):
        ...
        mask = cv.resize(mask, dsize=(self.nwidth, self.nheight), interpolation=cv.INTER_CUBIC)
        self.image_state = cv.resize(rgb, dsize=(self.nwidth, self.nheight), interpolation=cv.INTER_CUBIC)
    ...
    def reset(self, seed=None, options = None):
        ...
        target_names = ['red cube', 'yellow rubber duck', 'green apple']
        ...
        self.final_image = np.ones((self.nwidth, self.nheight, 4), dtype=np.uint8)
        self.step_count = 0
        self.done = False
